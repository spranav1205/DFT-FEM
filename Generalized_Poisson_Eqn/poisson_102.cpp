...
class Poisson
{
    public:
        ...
        AffineConstraints<double> constraints; // For hanging nodes
...
void Poisson<dim>::setup_system(int n_cells_per_edge, double start, double end, CoullombPotential<dim> &boundary_condition)
{
    ...
    constraints.close();
    cout<< "Number of constraints: " << constraints.n_constraints() << std::endl;
    
    // 102: make_sparsity_pattern
    DynamicSparsityPattern dsp(dof_handler.n_dofs(), dof_handler.n_dofs());
    DoFTools::make_sparsity_pattern(dof_handler, dsp, constraints, false); // false: no diagonal entries
...
int main()
{
    ...
    poisson_problem.setup_system(12, -10.0, 10.0, boundary_term);
    ...
    poisson_problem.output_results("temp.vtu", true);

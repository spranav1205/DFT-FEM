...
namespace Numerics
{
    ...
    void MatrixFreePoissonSolver<dim, fe_degree>::assemble_system(const Function<dim> &forcing_function)
    {
        ...
        // Store the unconstrained system_rhs for use in the FD preconditioner
        system_rhs_unconstrained = system_rhs;
    ...
    template <int dim, int fe_degree>
    int MatrixFreePoissonSolver<dim, fe_degree>::solve(bool verbose, bool custom_preconditioner)
    {

...
namespace Numerics
{
    // Matrix-free Laplace operator for CPU (MemorySpace::Host)
    template <int dim, int fe_degree, typename number = double>
    class LaplaceOperator
        : public dealii::MatrixFreeOperators::
              Base<dim, dealii::LinearAlgebra::distributed::Vector<number, dealii::MemorySpace::Host>>
    {
    public:
        using value_type = number;
        using VectorType = dealii::LinearAlgebra::distributed::Vector<number, dealii::MemorySpace::Host>;
